Feature: To check successful and unsuccessful Add Participant functionality by Site Admin in the application
  As an user of the application
  I want to test Add Participant by Site Admin feature

  Background: Login:LoginAsOrgAdmin,SearchPods:NavigateToDashFromPodOverview,ManageSites:AddSiteSuccess,ManageSiteAdmin:LoginAsSiteAdmin,SearchPods:NavigateToPodOverview,AddParticipant:AddUserToPodSuccess

Scenario: To verify success scenario for login with the user created by Site admin with DATA ChangePassword_Success
...
Scenario: To verify if the user created by site admin has same pod in ongoing pods with DATA PodSearch_Success
    Given I am ON participantResultList page
    Then I CLICK leftMenuButton icon
    And I CLICK logoutLink from menu options
    Then I am ON home page
...
Scenario: To verify if the user enrolled to pod by site admin is displayed in Manage user page of organization with DATA InviteUser_Success
    Given I am ON participantResultList page
...
Scenario: To verify if the user enrolled by site admin is displayed in the participant list page a pod in org admin with DATA InviteUser_Success
    Given I am ON participantResultList page
    Then I VERIFY ifUserIsEnrolled to the pod
    Then correct user is displayed on the page
    And I am ON participantResultList page
...
Scenario: To verify if the user created by site admin is removed by the org admin and status of the user is removed in participant list page of the pod in Site Admin with DATA InviteUser_Success
    Given I am ON participantResultList page
    Then I VERIFY ifUserIsEnrolled to the pod
    Then correct user is displayed on the page
    And I am ON participantResultList page
