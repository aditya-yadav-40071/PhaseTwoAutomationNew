...
final class OngoingTrainingPage extends WebPage {
	...
	def populateData = { browser, formKey, formData ->
		if(formKey.equals("searchInputs")){
			...
			new OngoingTrainingFilterForm().populateFields(browser, formData);
		}
	}

	def static getAllOngoingPodNames = { browser, formData ->
		new OngoingTrainingFilterForm().getAllOngoingPodNames browser, formData
	}

	def static getTotalViewingPodsCount ={ browser, formData ->
		new OngoingTrainingFilterForm().getTotalViewingPodsCount browser, formData
	}

	def static searchOngoingTrainings = { browser, formData ->
		new OngoingTrainingFilterForm().searchOngoingTrainings browser, formData
	}
	...
	static final class OngoingTrainingFilterForm extends WebForm {
		...
		private static final def POD_SEARCHTEXTBOX = "//div[@aria-hidden='false']//input[@ng-model='searchValue.skills']"
		...
		def static final ongoingTrainingsPageErrorMessageMap = []

		//To enter data
		def static final populateFields = { browser, formData ->
			println ("OngoingTrainingFilterForm.populateFields - data: " + formData)
		...
		def static searchOngoingTrainings = { browser, formData ->
			browser.delay(2000)
			if(browser.isDisplayed(POD_SEARCHTEXTBOX)){
				browser.populateField(POD_SEARCHTEXTBOX,formData[0])
				browser.delay(7000)
				return new SuccessOutcome()
			}else{
				return KPCommonPage.returnFailureOutcome(browser, "SearchTextBoxDisplyIssue", "The pod search textbox was not displayed")
			}
		}
		...
		def static getAllOngoingPodNames = { browser, formData ->
			...
			browser.delay(1000)
			if(browser.isDisplayed(POD_LIST)){
				KPCommonPage.allOngoingPodsList = browser.getLists(POD_LIST)
				return new SuccessOutcome()
			}else{
				return KPCommonPage.returnFailureOutcome(browser, "podListDisplayIssue", "No pods were displayed on the page")
			}
		}
	}
