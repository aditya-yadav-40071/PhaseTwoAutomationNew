...
final class RecommendedPodsPage extends WebPage {

	//Override
	def populateData = {browser, formKey, formData ->
		if(formKey.equals("searchInputs")){
			new RecommendedPodsSearchForm().populateFields(browser, formData);
		}
		else if(formKey.equals("filterDetails")){
			new RecommendedPodsFilterForm().populateFields(browser, formData);
		}
	}

	//To fetch the pod names if they are displaying
	def static getPodTextList = { browser, element, podList, fiftyRes ->
		new RecommendedPodsFilterForm().getPodTextList browser, element, podList, fiftyRes
	}


	//To get the total count of pods viewing per page
	def static getTotalViewingPodsCount ={ browser, formData ->
		new RecommendedPodsFilterForm().getTotalViewingPodsCount browser, formData
	}

	def static getFirstPodFromRecommendedList = {browser, formData ->
		new RecommendedPodsSearchForm().getFirstPodFromRecommendedList browser, formData
	}




	static final class RecommendedPodsSearchForm extends WebForm {
		...
		private static final def FIRST_POD = "//div[contains(@class,'clearfix content individual-pod mt10')][1]/div//h3/a"
		...
		def static final populateFields = { browser, formData ->
			println ("RecommendedPodsSearchForm.populateFields - data: " + formData)
				...
				for(int i = 0; i < FIELDS.size(); i++){
					...
					if(FIELDS[i].equals(SEARCH_TEXT)){
						if(KPCommonPage.getFirstPodName!=null){
							...
							KPCommonPage.podName = formData[i]
							println "=====FirstPodName====="+KPCommonPage.podName
		...
		def static final getFirstPodFromRecommendedList = {browser, formData ->
			if(browser.isDisplayed(POD_LIST)){
				KPCommonPage.podToBeCheckedInProgress = browser.gettext(FIRST_POD).trim()
	...
	static final class RecommendedPodsFilterForm extends WebForm {
		...
		private static final def FILTER_INDUSTRY = "//md-select[@ng-model='functionCall.filter.industry']" //"//div[@class='list-section resp_ml_20 resp_mr_20 ng-scope']/filter-list//md-select[@ng-model='functionCall.filter.industry']"

		private static final def FILTER_LEVEL = "//md-select[@ng-model='functionCall.filter.level']" //"//div[@class='list-section resp_ml_20 resp_mr_20 ng-scope']/filter-list//md-select[@ng-model='functionCall.filter.level']"
		...
		private static final def FILTER_MIN_DURATION = "//md-select[@ng-model='functionCall.filter.minDuration']" //"//div[@class='list-section resp_ml_20 resp_mr_20 ng-scope']/filter-list//md-select[@ng-model='functionCall.filter.minDuration']"

		private static final def FILTER_MIN_DURATION_LIST = "//md-option[@ng-value='minduration']"

		private static final def FILTER_MAX_DURATION = "//md-select[@ng-model='functionCall.filter.maxDuration']" //"//div[@class='list-section resp_ml_20 resp_mr_20 ng-scope']/filter-list//md-select[@ng-model='functionCall.filter.maxDuration']"
		...
		//To enter data
		def static final populateFields = { browser, formData ->
			println ("RecommendedPodsFilterForm.populateFields - data: " + formData)
			def outcome = WebForm.checkFormFieldsData(formData, FIELDS)
			if(outcome.isSuccess()){
				for(int i = 0; i < FIELDS.size(); i++){
					browser.delay(1000)
					if(FIELDS[i].equals(FILTER_SKILL)){
						if(!formData[i].equals("")){
							def viewingCount = RecommendedPodsPage.getTotalViewingPodsCount(browser,formData)
